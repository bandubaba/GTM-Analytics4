...
| A5b | **Orphan usage — out-of-window** | ~120 logs | Valid `account_id`, but `date` falls outside any active contract for that account. | Free-trial usage before contract start; test-system leakage; edge-case billing events. | Metric: **excluded**. DQ: **warning**; per-account count trended. |
