...
| D13 | **v0.7.1 metric calibration** — three parameter flips after attainment-chart review: `HEALTHY_U_MIN` 0.80→0.70 (healthy plateau widened so U ≥ 0.70 lands at `base = 1.00`), `SPIKE_DROP_MODIFIER` 0.70→0.50 (stronger penalty on the #1 churn-risk pattern), `EXPANSION_MODIFIER` 1.05→1.10 (mid-term expansion is a legible upside, not a rounding error) | Proposed | 2026-04-21 | Per-band attainment showed (i) healthy accounts at 70–80% utilization dragging rep attainment for no business reason, (ii) spike-drops landing at HS ≈ 0.70 — visually identical to mild under-use, (iii) expansion adding only $178K lift against a $133M book. All three are parameter flips; formula structure unchanged. Ramp slope auto-recalibrates from 1.20 to 1.50 to preserve continuity at the new plateau boundary. Reversible by flipping the three values back in `params.py`. | (i) leave plateau at [0.80, 1.10] and accept the systematic healthy-account drag; (ii) segment-specific spike-drop penalties (more knobs, per D01 we don't add knobs); (iii) capped expansion at HS 1.30 via a larger base bonus (changes the overage curve's meaning) | 03 |
